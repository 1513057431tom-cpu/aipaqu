...
`backend` 和 `frontend` 源码目录已经建立；新增模块必须继续遵循上述边界，并同步测试、迁移和环境模板。
...
- 导入内部数据、确认实体映射、生成每日情报快照和采购建议的端到端测试。
- 定向采集页面变化、确认外部信号并追溯原始证据的端到端测试。
- 从已批准日报结构化快照聚合周报并导出 DOCX 的端到端测试，断言未调用浏览器或内部同步。
