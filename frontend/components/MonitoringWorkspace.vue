...
async function collect(source: MonitoringSource) { collectingId.value = source.id; successMessage.value = ""; try { const result = await request<CollectionResult>(`/api/v1/sources/${source.id}/collect`, { method: "POST" }); if (result.job.status === "WAITING_HUMAN") successMessage.value = `${source.name} 已停止自动采集：${result.job.errorMessage || '需要人工授权或配置浏览器运行时'}。`; else if (result.signal) successMessage.value = `${source.name} 已完成 AI 分析并生成物料情报。`; else if (result.document?.previousContentDigest) successMessage.value = `${source.name} 采集成功，AI 未识别到与物料相关的新变化。`; else successMessage.value = `${source.name} 已建立证据基线，下次采集将进行变化分析。`; await loadAll(); emit("changed") } catch (error) { loadError.value = errorMessage(error, "采集启动失败。") } finally { collectingId.value = "" } }
